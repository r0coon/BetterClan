...
public record Settings(
        ...
        Set<ClanPermission> defaultMemberPermissions,
    ...
    private static Set<ClanPermission> loadPermissionSet(List<String> list, Set<ClanPermission> fallback) {
        if (list == null || list.isEmpty()) return fallback;
        Set<ClanPermission> result = EnumSet.noneOf(ClanPermission.class);
        for (String s : list) {
            try { result.add(ClanPermission.valueOf(s.toUpperCase())); } catch (IllegalArgumentException ignored) {}
        }

...
public class Clan {
    ...
    private static Set<Permission> DEFAULT_MEMBER_PERMISSIONS = EnumSet.of(
    ...
    public static void configure(int maxLog, int maxBanner, boolean defaultFF, Set<Permission> defaultMemberPerms) {
        MAX_LOG_ENTRIES = maxLog;
        MAX_BANNER_SLOTS = maxBanner;
    ...
    public Clan(String name, UUID ownerId) {
        ...
        initDefaultRanks();
        members.put(ownerId, Rank.OWNER_ID);
    }

    private void initDefaultRanks() {
        Rank owner = new Rank(Rank.OWNER_ID, "Anführer", "§c", false);
        owner.setPermissions(EnumSet.allOf(Permission.class));

        Rank member = new Rank(Rank.MEMBER_ID, "Mitglied", "§a", false);
        member.setPermissions(DEFAULT_MEMBER_PERMISSIONS);

        ranks.put(Rank.OWNER_ID, owner);
        ranks.put(Rank.MEMBER_ID, member);
    }
    ...
    public List<Rank> getOrderedRanks() {
        return ranks.values().stream()
                .sorted((a, b) -> {
                    if (Rank.OWNER_ID.equals(a.getId())) return -1;
                    if (Rank.OWNER_ID.equals(b.getId())) return 1;
                    return Integer.compare(b.getPermissions().size(), a.getPermissions().size());
                })
                .collect(Collectors.toList());
    }

    public void addCustomRank(Rank rank) {
    ...
    public boolean deleteRank(String rankId) {
        ...
        ranks.remove(rankId);
        members.replaceAll((uuid, rid) -> rid.equals(rankId) ? Rank.MEMBER_ID : rid);
        return true;
    }
    ...
    public String getDefaultOfficerRankId() {
        ...
        return ranks.values().stream()
                .filter(r -> !Rank.OWNER_ID.equals(r.getId()) && !Rank.MEMBER_ID.equals(r.getId()))
                .max(Comparator.comparingInt(a -> a.getPermissions().size()))
                .map(Rank::getId)
                .orElse(Rank.MEMBER_ID);
    }

    public String getDemotedRankId(String currentRankId) {
        if (currentRankId == null) return null;
        if (Rank.MEMBER_ID.equals(currentRankId)) return null;
        if (Rank.OWNER_ID.equals(currentRankId)) return getDefaultOfficerRankId();
    ...
    public Rank getEffectiveRank(UUID playerId) {
        String rankId = members.get(playerId);
        if (rankId == null) return null;
        Rank rank = ranks.get(rankId);

        return rank != null ? rank : ranks.get(Rank.MEMBER_ID);
    }
    ...
    public List<UUID> getOrderedMembers() {
        return members.entrySet().stream()
                .sorted((a, b) -> {
                    Rank ra = ranks.getOrDefault(a.getValue(),
                            ranks.getOrDefault(Rank.MEMBER_ID, null));
                    Rank rb = ranks.getOrDefault(b.getValue(),
                            ranks.getOrDefault(Rank.MEMBER_ID, null));
    ...
    public boolean hasPermission(UUID playerId, Permission perm) {
        if (ownerId.equals(playerId)) return true;
        Rank rank = getEffectiveRank(playerId);
        return rank != null && rank.hasPermission(perm);
    }

    public boolean rankHasPermission(String rankId, Permission perm) {
        Rank rank = ranks.get(rankId);
        return rank != null && rank.hasPermission(perm);
    }

    public void setRankPermission(String rankId, Permission perm, boolean enabled) {
        Rank rank = ranks.get(rankId);
        if (rank != null) rank.setPermission(perm, enabled);
    }

    @SuppressWarnings("unused")
    public Set<Permission> getRankPermissions(String rankId) {
        Rank rank = ranks.get(rankId);
        if (rank == null) return EnumSet.noneOf(Permission.class);
        if (Rank.OWNER_ID.equals(rankId)) return EnumSet.allOf(Permission.class);
    ...
    public void loadRanks(Map<String, Rank> loadedRanks) {
        ranks.clear();

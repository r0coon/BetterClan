...
public final class DatabaseStorage implements StorageProvider {

    private final DatabaseManager db;
    private final Logger log;

    public DatabaseStorage(DatabaseManager db, Logger log) {
        this.db = db;
        this.log = log;
        SchemaCreator.createTables(db, log);
    }
    ...
    private void loadClans(Connection conn, ClanManager manager) throws SQLException {
        ...
        try (var rs = conn.createStatement().executeQuery("SELECT * FROM gc_ranks")) {
            while (rs.next()) {
                ...
                CustomRank rank = new CustomRank(rid, rname, rcolor != null ? rcolor : "§7", del);
        ...
        try (var rs = conn.createStatement().executeQuery("SELECT * FROM gc_rank_permissions")) {
            while (rs.next()) {
                ...
                if (clan == null) continue;
                String rid = rs.getString("rank_id");
                CustomRank rank = clan.getRank(rid);
                if (rank == null) continue;
                try {
                    ClanPermission perm = ClanPermission.valueOf(rs.getString("permission"));
    ...
    private void loadWars(Connection conn, ClanManager manager) throws SQLException {
        List<ClanWar> wars = new ArrayList<>();
        Map<Integer, ClanWar> warsById = new HashMap<>();

        try (var rs = conn.createStatement().executeQuery("SELECT * FROM gc_wars")) {
            while (rs.next()) {
                int warId = rs.getInt("id");
                long timeout = rs.getLong("timeout_ms");
                if (timeout <= 0) timeout = ClanWar.DEFAULT_TIMEOUT_MS;
                ClanWar war = new ClanWar(rs.getString("clan1_name"), rs.getString("clan2_name"),
        ...
        try (var rs = conn.createStatement().executeQuery("SELECT * FROM gc_war_stats")) {
            while (rs.next()) {
                ClanWar war = warsById.get(rs.getInt("war_id"));
    ...
    private void loadWarRequests(Connection conn, ClanManager manager) throws SQLException {
    ...
    private void loadPlayerSettings(Connection conn, ClanManager manager) throws SQLException {
    ...
    private void loadVaults(Connection conn, ClanManager manager) throws SQLException {
    ...
    private void saveClans(Connection conn, ClanManager manager) throws SQLException {
    ...
    private void saveWars(Connection conn, ClanManager manager) throws SQLException {
    ...
    private void saveWarRequests(Connection conn, ClanManager manager) throws SQLException {
    ...
    private void savePlayerSettings(Connection conn, ClanManager manager) throws SQLException {
    ...
    private void saveVaults(Connection conn, ClanManager manager) throws SQLException {

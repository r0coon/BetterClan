...
public final class ClanListMenu {

    public static final int ITEMS_PER_PAGE = 28;

    private ClanListMenu() {}
    ...
    public static void openForWar(ClanManager manager, Player player, int page) {
        Clan playerClan = manager.getClan(player.getUniqueId());
        List<Clan> allClans = new ArrayList<>(manager.getAllClans());
    ...
    public static void openSearch(ClanManager manager, Player player, List<Clan> results, String query) {
        buildMenu(manager, player, results, 1, query, false);
    }

    private static void buildMenu(ClanManager manager, Player player, List<Clan> clans,
    ...
    public static void openInfo(ClanManager manager, Player player, Clan targetClan, int warTargetKills) {
        Settings settings = manager.getSettings();
        ...
        boolean canAlly = playerClan != null
                && manager.hasPermission(playerClan, player.getUniqueId(), ClanPermission.ALLY)
        ...
        boolean canWar = playerClan != null
                && manager.hasPermission(playerClan, player.getUniqueId(), ClanPermission.WAR)
        ...
        ClanWar activeWar = manager.getActiveWar(targetClan.getName());
        if (activeWar != null) {

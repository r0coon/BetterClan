...
public final class RankManagementMenu {

    private RankManagementMenu() {}

    public static void open(ClanManager manager, Player player) {
        Clan clan = manager.getClan(player.getUniqueId());
        if (clan == null) return;
        if (!manager.hasPermission(clan, player.getUniqueId(), com.betterclan.clan.ClanPermission.MANAGE_RANKS)) {
    ...
    public static void openForAdmin(ClanManager manager, Clan clan, Player player) {
        openForClan(manager, clan, player);
    }

    private static void openForClan(ClanManager manager, Clan clan, Player player) {
        ...
        List<CustomRank> ranks = clan.getOrderedRanks();
    ...
    static ItemStack buildRankItem(CustomRank rank) {
    ...
    static Material rankMaterial(CustomRank rank) {
        if (CustomRank.OWNER_ID.equals(rank.getId())) return Material.NETHER_STAR;

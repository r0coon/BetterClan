...
public final class MembersMenu {
    ...
    public static NamespacedKey memberKey(BetterClanPlugin plugin) {
        return new NamespacedKey(plugin, "clan-member-id");
    }

    public static void openForAdmin(BetterClanPlugin plugin, ClanManager manager, Player player, String clanName, int page) {
    ...
    public static void open(BetterClanPlugin plugin, ClanManager manager, Player player, int page) {
    ...
    private static void openWithClan(BetterClanPlugin plugin, ClanManager manager, Player player, Clan clan, String adminClanName, int page) {
        ...
        for (int i = 0; i < ITEMS_PER_PAGE && (startIndex + i) < ordered.size(); i++) {
            ...
            CustomRank role = clan.getEffectiveRank(memberId);
            ...
            if (role != null && CustomRank.OWNER_ID.equals(role.getId())) builder.glow();
    ...
    public static UUID extractMemberId(BetterClanPlugin plugin, ItemStack stack) {
    ...
    private static String formatLastSeen(long timestamp) {
        return MemberActionMenu.formatAge(timestamp);
    }
}

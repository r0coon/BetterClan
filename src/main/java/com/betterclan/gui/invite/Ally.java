...
public final class Ally {
    ...
    @SuppressWarnings("unused")
    public static void openList(Manager manager, Player player, int page) {
        Clan clan = manager.getClan(player.getUniqueId());
        if (clan == null) return;
        boolean canManage = manager.canManage(clan, player.getUniqueId());
    ...
    public static void openShields(Manager manager, Player player) {
        Clan clan = manager.getClan(player.getUniqueId());
        if (clan == null) return;
        boolean canManage = manager.canManage(clan, player.getUniqueId());
    ...
    public static void openRequests(Manager manager, Player player, int page) {
        Clan clan = manager.getClan(player.getUniqueId());
        if (clan == null) return;
        boolean canManage = manager.canManage(clan, player.getUniqueId());

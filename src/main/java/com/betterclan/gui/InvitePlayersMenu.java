...
public final class InvitePlayersMenu {

    public static final int ITEMS_PER_PAGE = 30;

    private InvitePlayersMenu() {}

    private static NamespacedKey targetKey(Plugin plugin) {
        return new NamespacedKey(plugin, "invite-target-id");
    }

    public static void open(Plugin plugin, ClanManager manager, Player player, int page) {
        open(plugin, manager, player, page, null);
    }

    public static void open(Plugin plugin, ClanManager manager, Player player, int page, String searchQuery) {
